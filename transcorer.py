...
class TranScorer:
    ...
    def __init__(self, ttab: TTable):
        self.src_tgt = ttab.fwd
        self.tgt_src = ttab.inv
        log.info(f"Translation Scorer source: {ttab.src} tgt:{ttab.tgt}")

    @classmethod
    def new(cls, ttab_path):
        return cls(TTable.load_from(ttab_path))

    @staticmethod
    def _translation_evidence(tok: str, ttab: Dict[str, Dict[str, float]], cand_toks: Set[str]):
        """
        ...
        return sum([probs[tok] for tok in probs.keys() & cand_toks])
